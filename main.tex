...
\[ M_{v_i}={G_h}^{r_{v_i}}{G_g}^{v_i} \qquad M_{s_i}={G_h}^{r_{s_i}}{G_g}^{s_i} \]
...
The user needs to convince the coordinator that the amounts redeemed and the amounts requested differ by the public input $\Delta_{v}$, which she can prove by including the following proof of knowledge:
\[
\pi^{\mathit{sum}} = \operatorname{PK}(\{ (z, r) : B = {G_v}^{z} {G_h}^{\Delta_r} \})
\]
where
\[
B = {G_g}^{\Delta_v} \prod_{i=1}^k \frac{C_{v_i}}{{M^\prime}_{v_i}}
\qquad
z = \sum_{i=1}^k z_i
\qquad
\Delta_r = \sum_{i=1}^k r_{v_i} - r^{\prime}_{v_i}
\]
with $r^{\prime}_{v_i}$ denoting the randomness in the $({M^{\prime}_{v_i})_{i=1}^k$ attributes of the credentials being requested and $z_i, r_{v_i}$ denoting the ones in the randomized attributes $(C_{v_i})_{i=1}^k$ of the credentials being presented.
...
To unconditionally preserve user privacy in the event that the hardness assumption of the discrete logarithm problem in $\mathbb{G}$ is broken we can add an additional randomness term $r_{s_i}^{\prime}$ used with an additional generator $G_h^{\prime}$ to the serial number commitments $M_{s_i}$, and similarly another randomness term $z_i^{\prime}$ and generators $G_v^{\prime}, G_s^{\prime}, G_{x_0}^{\prime}, G_{x_1}^{\prime}, G_V^{\prime}$ in order to obtain unconditional unlinkability for the commitments.\footnote{Assuming the coordinator is not able to attack network level privacy and the proofs of knowledge are unconditionally hiding.}

% TODO are the value commitments also an issue? what about for initial credentials where $M_{v_i} = G_h^{r_{v_i}}$? do we gain anything from having a single multi-commitment attribute?
