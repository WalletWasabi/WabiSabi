...
\begin{figure}[h!]
    ...
    \begin{enumerate}
        ...
        \item The coordinator verifies proofs and registers requested output if all proofs are valid and the serial numbers have not been used before.
    \end{enumerate}
\end{mdframed}
...
A malicious participant still learns all the input and output registrations\footnote{If \texttt{SIGHASH\_ANYONECANPAY} is set in the signature flags the full set of inputs could be kept known only to the coordinator until all signatures have been provided.} including amounts and scripts, so denial of service also presents privacy concerns.
